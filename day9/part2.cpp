...
public:
    ...
    */
    ...
    /**
    * Add a roll to the beginning
    * @param n The roll to add
    */
    void add(const int n) {
        rolls.insert(rolls.begin(), n);
    }
};
    ...
    */
    [[nodiscard]] int find_next() {
        for (int i = rolls.size() - 2; i >= 0; i--) {
            rolls.at(i).add(rolls.at(i).get_rolls().front() - rolls.at(i + 1).get_rolls().front());
        }
        return rolls.front().get_rolls().front();
    }
};
...
int main() {
    ...
    for (FullRoll& full_roll : full_rolls) {
        const int next = full_roll.find_next();
        sum += next;
    }
