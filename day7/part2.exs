defmodule CardCombinations do
  def generate_combinations(cards) do
    Enum.reduce(cards, [""], fn card, acc ->
      combine_values(card, acc)
    end)
    |> Enum.reverse()
  end

  defp combine_values(card, acc) do
    Enum.flat_map(acc, fn combination ->
      Enum.map(card, &("#{combination}#{&1}"))
    end)
  end
end
...
defmodule Hand do
  ...
  def process_hand(hand) do
    Enum.map(0..4, fn x ->
      if String.at(hand, x) == "J" do
        ["A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2", "J"]
      else
        [String.at(hand, x)]
      end
    end) |> CardCombinations.generate_combinations |> Enum.map(fn x -> process_cards(count_cards(x)) end) |> Enum.max
  end
